...
class ModNumber {
  public:
    ...
    ModNumber(T value_)
      : value(validate(value_)) {}

    void operator=(const ModNumber& other) {
      value = validate(other.value);
    }
...
template <class Number, class Point>
class EllipticCurve {
  public:
    EllipticCurve(Number a_, Number b_) 
      : a(a_), b(b_) {}
